...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterRequest) => {
    ...
    try {
      ...
      if (!err.response) {
        setServerError('Sunucuya bağlanılamadı. Lütfen backend (http://localhost:5000) servisinin çalıştığından emin olun.');
      } else {
        const msg = err.response?.data?.message || (typeof err.response?.data === 'string' ? err.response?.data : null) || 'Kayıt oluşturulamadı. Lütfen bilgilerinizi kontrol ediniz.';
        setServerError(msg);
      }
    }
  };
      ...
      <div className="w-full max-w-md space-y-4">
        {needAuth && (
          <div className="p-4 rounded-2xl bg-indigo-950/60 border border-indigo-800/60 text-indigo-200 text-xs flex items-start gap-3 shadow-lg">
            ...
            <div>
              <span className="font-bold block text-white mb-0.5">Hesap Oluşturun</span>
              Proje eklemek, problem-çözüm günlüğü tutmak ve analitik çizelgenizi oluşturmak için lütfen kayıt olun.
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 bg-slate-900/90 p-8 rounded-3xl border border-slate-800 shadow-2xl">
          <div className="text-center space-y-1">
            ...
            <h1 className="text-2xl font-bold text-white tracking-tight pt-2">Kayıt Ol</h1>
            <p className="text-xs text-slate-400">Ücretsiz geliştirici hesabı oluşturun</p>
          </div>

          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Görünen Ad / Ad Soyad *</label>
              <input
                placeholder="ör. Ali Efe"
                className="input"
                {...register('displayName', { required: 'Ad gerekli' })}
              />
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Şifre * (en az 8 karakter)</label>
              <input
                type="password"
                placeholder="••••••••"
                className="input"
                {...register('password', { required: 'Şifre gerekli', minLength: { value: 8, message: 'En az 8 karakter' } })}
              />
